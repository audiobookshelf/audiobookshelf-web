...
At this point the call chain is `/api/me` or `/api/users/{userID}` → `/api/items/{bookID}` or `/api/items/{bookID}/metadata-object` → ISBN with an optional `/api/search/book` if the server doesn't already have it. Now we are ready for importing into 3rd party services.
...
By default imported books will be placed in the `Read` shelf unless otherwise specified in a `Shelves` column. Other default options are `to-read` and `currently-reading`. Shelves are Goodreads equivilent to tags and should be treated as such with the 3 default ones being mutually exclusive. Spaces in existing tags should be replaced by dashes as Goodreads treats the `Shelves` entry as a list of space seperated tags.
