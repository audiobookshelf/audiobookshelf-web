...
The source of truth for everything you can do to your ABS server can be found [here](https://api.audiobookshelf.org).
...
Windows CMD 
```cmd
curl http://localhost:13378/audiobookshelf/api/items/{book id}/chapters ^
...
Linux Bash
```bash
curl -X POST http://localhost:13378/audiobookshelf/api/items/{book id}/chapters \
  -H "Authorization: Bearer user.api.token.here" \
  -H "Content-Type: application/json" \
  -d '{"chapters": [{"id":0,"start":0,"end":150.296003,"title":"Introduction"}]}'
```
...
## Convert Exported Lables to TSV
...
## Convert TSV to JSON
- [This](https://csvjson.com/csv2json) converter is convenient and also a cli tool if that fits your workflow
- Output: Array > Minify
...
## Pulling your data
Either use curl or navigate to the user endpoint in a browser `/audiobookshelf/api/users/{user-id}`. Alternatively if you are interested in your own data you can use the `/audiobookshelf/api/me` endpoint.
...
```json
{
  ...
  "mediaItemId": "b3enbaf6-fvb0-4v7c-bfsb-29rw51364a68",    //The ID of the internal book/podcast entry
  "mediaItemType": "book",                                  //Is it a book or podcastEpisode
  "duration": 77192.973333,                                 //How long the book is
  "progress": 1,                                            //How far the user got as a %
  ...
  "coverPath": "T:/he/path_to/your_cover_art.jpg",          //
...
- `displayTitle` - The title of the book 


> If you need more data points for matching books in other services try checking `/audiobookshelf/api/items/{itemID}/metadata-object` before pulling the whole library item entry.
---
# Encode Book as M4B
