...
Audnex.us is already used internally in ABS and often times will have an ISBN in their book [endpoint](https://audnex.us/#tag/Books/operation/getBookById) `api.audnex.us/books/{ASIN}`

Please be kind to this api by throttling your requests as it is community run and a great resource for us all. ABS already has rate limiting built into it for this reason and thus makes automating very easy for us. We can use the ABS search [endpoint](https://api.audiobookshelf.org/#search-for-books) with `provider=audnexus` to get results containing an ISBN.

At this point the call chain is `/api/me` or `/api/users/{userID}` → `/api/items/{bookID}` or `/api/items/{bookID}/metadata-object` → ISBN with an optional `/api/search/book` if the server doesn't already have it. No we are ready for importing into 3rd party services.

> ℹ️ The ASIN can be used in the title parameter though sanity checking that there is a description or publisher is a good way to toss out [strange results](your.abs.server/audiobookshelf/api/search/books?title=B002VA3LF8&author=Diana%20Gabaldon&provider=audnexus)
...
The most basic import can be just a .txt file or even pastebin link with ISBNs on each line for books you've read.
...
> ℹ️ It mentions tags as well but does not specify them as an accepted column so your mileage may vary

By default imported books will be placed in the `Read` shelf unless otherwise specified in a `Shelves` column. Other default options are `to-read` and `currently-reading`

Some filtering may be required as their instructions specify to seperate multiple tags/multiple shelves with spaces as well as putting double quotes around any entry that contains commas (ie Titles or Reviews)

[todo]: <> (Library Thing)
[todo]: <> (Essentially the same, ISBN only. Need to look more into this.)
